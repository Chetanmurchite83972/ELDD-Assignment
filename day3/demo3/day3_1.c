...
static __init int desd_init(void)
{
	...
	printk(KERN_INFO "%s : desd_init() : initialization is successful\n", THIS_MODULE->name);
	return 0;
}
...
static __exit void desd_exit(void)
{
	...
	printk(KERN_INFO "%s : desd_exit() : deinitialization is successful\n", THIS_MODULE->name);
}
...
MODULE_DESCRIPTION("This is revision module");
